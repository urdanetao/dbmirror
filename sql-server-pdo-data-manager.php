<?php
    /**
     * Clase SqlServerPDODataManager
     * 
     * Clase para manejar bases de datos SQL Server.
     * Dev. Oscar E. Urdaneta
     * Fecha: 2024-01-29
     */
    class SqlServerPDODataManager {
        /**
         * Definicion de constantes.
         */

        // Nombre del archivo de registro de eventos.
        private const __SQL_SERVER_DATA_MANAGER_LOG_FILENAME = 'sql-server-pdo-data-manager.log';
        ...
        function __construct($dbInfo) {
            ...
            try {
                $conn = PDO("sqlsvr:host=$host;dbname=$dbname", $user, $pwd);
         ...
         * 
         * @param e - string ó un objeto de la clase PDOException.
         * 
        ...
        private function ErrorHandler($e) {
            if ($this->saveEventLog) {
                if (is_a($e, 'PDOException')) {
                    $this->errorMessage = strval($e->getCode()) . ' - ' . $e->getMessage();
                } elseif (gettype($e) == 'array') {
                    $this->errorMessage = strval($e[0]) . ' - ' . $e[1] . ' - ' . $e[2];
                } elseif (gettype($e) == 'string') {
                    $this->errorMessage = $e;
                } else {
                    $this->errorMessage = 'ErrorHandler: Parametro $e de tipo no soportado -> ' . gettype($e);
                }
        ...
        public function Query($sqlCommand, $hasData = true, $maxRows = 0) {
            ...
            $stmt = $this->conn->query($sqlCommand, PDO::FETCH_ASSOC, false);

            // Si falla la ejecusion.
            if ($stmt === false) {
                $this->ErrorHandler($this->conn->errorInfo());
                return false;
            }
